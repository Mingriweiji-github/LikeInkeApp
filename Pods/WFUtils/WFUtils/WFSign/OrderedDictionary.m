...
+ (instancetype)dictionaryWithDictionary:(NSDictionary *)dict {
    if (dict && [dict isKindOfClass:[NSDictionary class]]) {
        __autoreleasing id dic = [[self alloc] initWithObjects:dict.allValues forKeys:dict.allKeys];
        return dic;
    }
    else {
        return nil;
    ...
    if ((self = [super init]))
    {
        ...
        for (id value in objects) {
            if ([value isKindOfClass:[NSDictionary class]]
                && ![value isKindOfClass:[OrderedDictionary class]]) {
                OrderedDictionary *dic = [OrderedDictionary dictionaryWithDictionary:value];
                NSInteger index = [tempObjects indexOfObject:value];
                tempObjects[index] = dic;
                continue;
            }
            else if ([value isKindOfClass:[NSArray class]]) {
                NSArray *array = [value toArrayWithOrderdDictionarys];
                NSInteger index = [tempObjects indexOfObject:value];
                tempObjects[index] = array;
                continue;
            }
        }
        _values = [tempObjects copy];
...
- (NSArray *)allKeys
{
    return [_keys sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:nil ascending:YES]]];
}
